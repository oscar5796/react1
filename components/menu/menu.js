...
class Menu extends React.Component{
    ...
    render(){
		const items = [
    		{id: 1, name: 'First', numToAdd:10, path:'result'},
    		{id: 2, name: 'Second', numToAdd:20, path: 'OK'},
    		{id: 3, name: 'Third', numToAdd:30, path: '/'}
		];

		const itemList = items.map( (item)=>{

		    return <MenuItem path={item.path} itemId={item.id} numToAdd={item.numToAdd} key={item.id} name={item.name} />
		});
